...
class MataPelajaranViewModel(
    ...
    fun buatMataPelajaran(namaPelajaran: String, deskripsi: String) = defaultScope.launch {
        repository.buatMataPelajaran(namaPelajaran, deskripsi)
            .catch { _buatMatpel.emit(State.Failed(getHttpException(it))) }
    ...
    fun fetchMataPelajaran() = defaultScope.launch {
        repository.getMataPelajaran()
            .catch { _fetchMatpel.emit(State.Failed(getHttpException(it))) }
    ...
    fun uploadMateri(idMatpel: String, file: File) = defaultScope.launch {
        repository.uploadMateri(idMatpel, file)
            .catch { _upload.emit(State.Failed(getHttpException(it))) }
